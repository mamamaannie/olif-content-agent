...
- [references/creator-profile.md](references/creator-profile.md) for the creator, audience, positioning, cases, and fact boundaries.
- [references/content-method.md](references/content-method.md) for the learned short-video principles and quality bar.
...
- Choose the content job yourself: traffic, trust, or conversion. A traffic piece may end cleanly without a sales CTA.
- Choose an appropriate length from roughly 15–35 seconds, 45–90 seconds, or 2–8 minutes. Do not force every idea into 60 seconds.

## Research when the claim is current

Browse the web whenever the user asks for current topics, recent benchmark performance, platform discussion, news, policies, companies, people, or other time-sensitive facts. Prefer primary reporting, official data, and original posts.
...
Lead with the judgment or recommended direction. Default to one strongest topic and one strongest opening. Provide alternatives only when the user asks or when two genuinely different strategic tests are warranted.

For a mature script request, normally include:

- content job and recommended length;
- one title/opening;
- complete spoken script;
- a compact shooting card with natural delivery beats;
- only the fact confirmations that still matter.

Do not force these sections onto a simple brainstorm or follow-up question. Keep the language conversational, specific, slightly funny, and quotable without sacrificing trust.

## Close the learning loop

When publication data is provided, diagnose the most likely bottleneck from the content's intended job. Prefer patterns across 10–20 posts over declaring success or failure from one upload. Track user rewrites, phrases they would never say, and verified performance signals as future calibration material when the user asks to update the skill.
